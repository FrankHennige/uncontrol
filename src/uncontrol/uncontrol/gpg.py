...
from gnupg import GPG as BaseGPG
...
class GPG(BaseGPG):
    ...
    def __init__(self, config: GPGConfig) -> None:
        """
    ...
    def encrypt(self, message: str | bytes, **kwargs) -> str:  # pylint: disable=arguments-differ
        ...
        """
        recipients = self.config.GPG_RECIPIENTS
        if isinstance(recipients, str):
            recipients = self.config.GPG_RECIPIENTS.split(",")
